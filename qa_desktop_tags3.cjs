...
(async () => {
  ...
  const tagsLayout = await page.evaluate(() => {
    ...
    return nodes.map((node, i) => {
      ...
      // Get the next sibling element after the label
      let media = label.nextElementSibling;
      if (media && media.classList.contains('drag-handle')) {
        media = media.nextElementSibling; // Skip drag handle if present
      }
